...
import { followUpApi, labCrmApi } from '@/lib/api';
...
function FollowUpForm({ initial, labId, onSave, onClose }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.scheduledAt) return toast.error('Date/time required');
    setLoading(true);
    try {
      if (initial?._id) await followUpApi.update(initial._id, form);
      else await followUpApi.create({ ...form, lab: labId });
      toast.success(initial ? 'Updated!' : 'Scheduled!');
      onSave();
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {initial && (
        <div>
          ...
          <textarea value={form.outcome} onChange={(e) => setForm({ ...form, outcome: e.target.value })} className="input" rows={2} placeholder="What happened..." />
        </div>
      )}
...
export default function LabCRMFollowUpsPage() {
  ...
  const fetchFollowUps = useCallback(() => {
    if (!labId) return;
    setLoading(true);
    followUpApi.getAll({ page, limit, lab: labId, status: statusFilter || undefined, today: todayOnly ? 'true' : undefined })
      .then((res) => { setFollowUps(res.data.items || []); setTotal(res.data.total || 0); })
      .finally(() => setLoading(false));
  }, [page, limit, labId, statusFilter, todayOnly]);

  useEffect(() => { fetchFollowUps(); }, [fetchFollowUps]);

  const markDone = async (fu) => {
    try { await followUpApi.update(fu._id, { status: 'done' }); toast.success('Done!'); fetchFollowUps(); }
    catch (err) { toast.error(getErrorMessage(err)); }
  };

  const handleDelete = async (id) => {
    if (!confirm('Delete?')) return;
    try { await followUpApi.delete(id); toast.success('Deleted'); fetchFollowUps(); }
    catch (err) { toast.error(getErrorMessage(err)); }
  };
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Schedule and track your patient/lead follow-ups</p>
        </div>
            ...
            <div key={fu._id} className={`card flex items-start gap-4 ${isOverdue(fu) ? 'border-red-200 bg-red-50/30' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-medium text-gray-900">{fu.patient?.name || fu.lead?.name || 'Unknown'}</p>
                    <p className="text-sm text-gray-500">{fu.patient?.mobile || fu.lead?.mobile}</p>
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${STATUS_COLORS[fu.status]}`}>{fu.status}</span>
                </div>
                <p className={`text-xs mt-1 flex items-center gap-1 ${isOverdue(fu) ? 'text-red-600 font-medium' : 'text-gray-400'}`}>
                  <FiClock size={11} />{isOverdue(fu) ? 'OVERDUE · ' : ''}{formatDate(fu.scheduledAt)}
                </p>
                {fu.notes && <p className="text-sm text-gray-600 mt-1">{fu.notes}</p>}
              </div>
              <div className="flex gap-1 shrink-0">
                {fu.status === 'pending' && (
                  <button onClick={() => markDone(fu)} className="text-green-500 hover:text-green-700 p-1.5 hover:bg-green-50 rounded-lg"><FiCheck size={15} /></button>
                )}
                <button onClick={() => setModal({ type: 'edit', followUp: fu })} className="text-gray-400 hover:text-primary-600 p-1.5 hover:bg-gray-100 rounded-lg"><FiEdit size={14} /></button>
                <button onClick={() => handleDelete(fu._id)} className="text-gray-400 hover:text-red-500 p-1.5 hover:bg-red-50 rounded-lg"><FiTrash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={modal?.type === 'add'} onClose={() => setModal(null)} title="Schedule Follow-up">
        <FollowUpForm labId={labId} onSave={() => { setModal(null); fetchFollowUps(); }} onClose={() => setModal(null)} />
      </Modal>
      <Modal open={modal?.type === 'edit'} onClose={() => setModal(null)} title="Edit Follow-up">
        <FollowUpForm initial={modal?.followUp} labId={labId} onSave={() => { setModal(null); fetchFollowUps(); }} onClose={() => setModal(null)} />
      </Modal>
    </div>
  );
}
