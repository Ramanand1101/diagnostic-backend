...
function ScheduleForm({ onSave, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    lab: '', package: '', slotDate: '', slotTime: '', notes: '',
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {a.status === 'confirmed' && (
          <>
            <button disabled={busy} onClick={() => run(() => corporateAppointmentApi.notifyEmployee(a._id, ['email']), 'Confirmation emailed')} className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:border-primary-300 flex items-center gap-1"><FiMail size={11} /> Notify (Email)</button>
            <button disabled={busy} onClick={() => run(() => corporateAppointmentApi.notifyEmployee(a._id, ['whatsapp']), 'Confirmation sent on WhatsApp')} className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:border-primary-300 flex items-center gap-1"><FiPhone size={11} /> Notify (WhatsApp)</button>
          </>
        )}
      ...
      {showReschedule && (
        <div className="bg-gray-50 rounded-lg p-3 space-y-2">
          ...
          <input value={rescheduleForm.reason} onChange={(e) => setRescheduleForm((f) => ({ ...f, reason: e.target.value }))} className="input text-sm" placeholder="Reason (optional)" />
          ...
          <div className="space-y-1">
            {a.rescheduleHistory.map((h, i) => (
              <p key={i} className="text-xs text-gray-500">
                {h.fromDate ? new Date(h.fromDate).toDateString() : '—'} {h.fromTime} → {h.toDate ? new Date(h.toDate).toDateString() : '—'} {h.toTime} {h.reason ? `(${h.reason})` : ''}
              </p>
            ))}
          </div>
        </div>
